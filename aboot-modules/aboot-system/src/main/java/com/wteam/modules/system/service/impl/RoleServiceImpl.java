...
public class RoleServiceImpl implements RoleService {
    ...
    List<Long> DEFAULT_ROLES = Arrays.asList(1L, 2L, 3L);
    ...
    public void update(Role resources) {
        ...
        if (DEFAULT_ROLES.contains(role.getId())) {
            throw new BadRequestException("无法修改初始角色!");
        }
    ...
    public void delete(Set<Long> ids) {
        if (ids.containsAll(DEFAULT_ROLES)) {
            throw new BadRequestException("无法删除初始角色!");
        }
    ...
    public void updatePermission(Role resources) {
        ...
        if (role.getId() == 1L) {
            throw new BadRequestException("无法修改\"超级管理员\"角色!");
        }
